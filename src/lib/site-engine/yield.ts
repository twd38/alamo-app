...
export function buildYield(lot: Lot, scheme: SchemeTemplate): YieldResult {
  ...
  const buildDepth = Math.max(
    lot.depthFt - lot.setbacks.front - lot.setbacks.rear,
    0
  );
  ...
  if (scheme.name === 'Rowhouses') {
    const rowsThatFit = Math.floor(
      buildWidth / (scheme.footprintPerUnitSqFt / buildDepth)
    );
  ...
  const maxStoriesByHeight = lot.heightLimitFt
    ? Math.floor(lot.heightLimitFt / 10)
  ...
  while (
    (gfa / lot.areaSqFt > farLimit ||
      footprint / lot.areaSqFt > coverageLimit) &&
    stories > 0
  ) {
    stories -= 1;
  ...
  const stalls = Math.ceil(
    units * Math.max(lot.parkingMinPerUnit, scheme.parkingPerUnit)
  );
