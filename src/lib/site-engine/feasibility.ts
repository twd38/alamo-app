...
export function checkFeasibility(lot: Lot, scheme: SchemeTemplate): FeasibilityResult {
  const b: string[] = [];

  if (lot.areaSqFt < scheme.minLotAreaSqFt) b.push("lot too small");
  if (lot.widthFt < scheme.minLotWidthFt) b.push("lot too narrow");
  if (lot.unitLimit !== null && scheme.baseUnits > lot.unitLimit) b.push("unit cap exceeded");
  ...
  const stallsNeeded = scheme.baseUnits * Math.max(lot.parkingMinPerUnit, scheme.parkingPerUnit);
