...
export async function generatePartType(
  componentPartCategories: PartType[],
  ...
  if (
    componentPartCategories.some((partType) => partType == PartType.MODULE_300)
  ) {
    partType = PartType.ASSEMBLY_400;
  } else if (
    componentPartCategories.some(
      (partType) => partType == PartType.SUBASSEMBLY_200
    )
  ) {
    partType = PartType.MODULE_300;
  } else if (
    componentPartCategories.some((partType) => partType == PartType.PART_100)
  ) {
...
export async function generateNewPartNumbers(
  componentPartCategories: PartType[],
  isRawMaterial: boolean
) {
  ...
  const partType = await generatePartType(
    componentPartCategories,
    isRawMaterial
  );
  ...
  const sequenceBaseNumber = lastBasePartNumber
    ? parseInt(lastBasePartNumber.partNumber.split('-')[0])
    : 10000000;
  ...
  const partTypeNumberWithVersion =
    parseInt(partTypeNumber) + parseInt(versionNumber);
...
export function detectCarrierAndTrackingURL(trackingNumber: string) {
  const carriers = [
    ...
    },
    {
      ...
      url: (tn: string) =>
        `https://tools.usps.com/go/TrackConfirmAction?qtc_tLabels1=${tn}`
    },
    {
      name: 'DHL',
    ...
    },
    {
      ...
      url: (tn: string) =>
        `https://www.amazon.com/progress-tracker/package/${tn}`
    }
  ];
