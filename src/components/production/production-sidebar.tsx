...
export function ProductionSidebar({ step, workOrder, onStepCompleted }: ProductionSidebarProps) {
    ...
    const stepExecution = step ? workOrder?.stepExecutions?.find(se => 
        se.workInstructionStep.id === step.id
    ) : null;
    ...
    const allActionsCompleted = requiredActions.length === 0 || 
        completedRequiredActions.length === requiredActions.length;
    
    const canCompleteStep = isWorkOrderInProgress && allActionsCompleted && !isStepCompleted;
            ...
            <div className="border-t border-inherit px-4 py-4 flex-shrink-0">
                ...
                {!allActionsCompleted && isWorkOrderInProgress && !isStepCompleted && (
                    <div className="mb-2 text-sm text-blue-600 bg-blue-50 p-2 rounded">
                        Complete {requiredActions.length - completedRequiredActions.length} more required actions
                    </div>
                )}
                <Button 
                    variant="default" 
                ...
                >
                    ...
                    {isCompleting ? "Completing..." : isStepCompleted ? "Step Completed" : "Complete Step"}
                </Button>
            </div>
        </div>
    );
