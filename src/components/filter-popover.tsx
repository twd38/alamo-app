...
const defaultFilterStateAtom = atomWithStorage<FilterState>('defaultFilters', {
  filters: []
});
...
const atomCache = new Map<
  string,
  ReturnType<typeof atomWithStorage<FilterState>>
>();
...
const ValueInput = ({
  ...
  filter: FilterItem;
  filterOptions: FilterOption[];
  updateFilter: (id: string, field: keyof FilterItem, value: string) => void;
}) => {
    ...
    return (
      <Switch
        ...
        onCheckedChange={(checked: any) =>
          updateFilter(filter.id, 'value', checked.toString())
        }
      />
    );
  }
...
export function useFilterAtom(storageKey: string) {
  const filterStateAtom = storageKey
    ? getAtomForKey(storageKey)
...
export function FilterPopover({
  ...
  const getIconForType = (type: string) => {
    ...
    return (
      option?.icon ||
      filterOptions[0]?.icon || <Filter className="h-4 w-4 mr-2" />
    );
  };
      ...
      <PopoverContent className="w-[600px] p-4" align="end">
              ...
              <div key={filter.id} className="flex items-center gap-2">
                    ...
                    <div className="max-h-[300px] overflow-auto">
                      {filterOptions.map((option) => (
                        <div
                          ...
                          onClick={() =>
                            updateFilter(filter.id, 'type', option.label)
                          }
                        >
                          {option.icon}
                          {option.label}
                ...
                <Popover>
                  <PopoverTrigger asChild>
                    <Button
                      variant="outline"
                      className="w-[100px] justify-between border border-gray-200"
                    >
                  ...
                  <PopoverContent className="w-[200px] p-0" align="start">
                    {operatorOptions.map((option) => (
                      <div
                        ...
                        onClick={() =>
                          updateFilter(filter.id, 'operator', option.label)
                        }
                      >
                        {option.label}
                      </div>
        ...
        {onSaveFilters && (
          <div className="flex justify-end gap-2 pt-4 border-t mt-2">
            <Button variant="outline" onClick={() => onSaveFilters(filters)}>
              Save Filters
            </Button>
          </div>
        )}
