...
// Using any for step type to support both WorkInstructionStep and WorkOrderWorkInstructionStep
...
interface WorkInstructionContentProps {
  step: any; // Can be WorkInstructionStep or WorkOrderWorkInstructionStep
  onUpdateStep: (stepId: string, updates: any) => void;
}

export const WorkInstructionContent: React.FC<WorkInstructionContentProps> = ({
  step,
  onUpdateStep
}) => {
  ...
  // Separate local state for title to prevent typing interference
  ...
  // Watch for form changes (estimated labour time only) and trigger auto-save
  useEffect(() => {
    const subscription = form.watch((value, { name, type }) => {
      if (type === 'change' && name === 'estimatedLabourTime') {
        setIsEditing(true);
        setSaveError(false);
