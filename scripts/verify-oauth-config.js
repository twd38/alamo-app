...
function main() {
  ...
  if (envInfo.vercelEnv === 'preview') {
    ...
    console.log('Consider adding one of these options to Google Cloud Console:');
    console.log('  1. Wildcard: https://*.your-domain.com/api/auth/callback/google');
    console.log('  2. Specific URL: ' + envInfo.redirectUri);
    console.log('  3. Use production domain by setting NEXTAUTH_URL in preview environment');
  }
  
  console.log('\n🐛 Debug Endpoint:');
  console.log(`Visit ${envInfo.baseUrl}/api/auth/debug for runtime configuration`);
